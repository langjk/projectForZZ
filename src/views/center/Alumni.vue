            ...
            <el-row style="width: 100%; align-items: center">
                ...
                <el-col :span="18">
                    ...
                    <el-row style="width: 100%; margin: 10px 0; font-size: 90%">
                        <el-col :span="14"
                            ><strong>入学年份 </strong> {{ item.enrollment_year }}</el-col
                        >
                        <el-col :span="10"
                            ><strong>毕业年份 :</strong>
                            {{ item.graduation_year }}</el-col
                        >
                    </el-row>
                    <span><strong>毕业去向 </strong> {{ item.destination }}</span>
                </el-col>
            </el-row>
        </div>
    </div>
...
const loadMember = async () => {
    const memberPath = import.meta.env.BASE_URL + "/Alumni/alumni.json";

    const memberResponse = await fetch(memberPath);
    const members = await memberResponse.json();

    for (const member of members) {
        const imagePath = `${basePath}Alumni/${member.name}.png`;
        // 将 JSON 数据和图片路径加入列表
        rawData.value.push({
            ...member,
            image: imagePath,
